...
exports.registrar = async (req, res) => {
    ...
    try {
        ...
        console.log(`>> CODIGO ASIGNADO A ${email}: [${codigo}]`);
        ...
        if (transporter) {
            await transporter.sendMail({
                ...
                html: `<div style="text-align:center"><h1>${codigo}</h1><p>Ingresa este código en la app.</p></div>`
            });
        }
...
exports.verificarCuenta = async (req, res) => {
    ...
    try {
        ...
        if (!usuario) {
            return res.status(400).json({ msg: 'Usuario no encontrado. Regístrate nuevamente.' });
        }

        const tokenBD = String(usuario.token).trim();
        const tokenInput = String(codigo).trim();

        if (tokenBD !== tokenInput) {
            console.log(`FALLO VERIFICACIÓN: BD[${tokenBD}] vs INPUT[${tokenInput}]`);
